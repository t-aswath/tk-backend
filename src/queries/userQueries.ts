...
`;
export const deleteAllFromCart: string =
  "DELETE FROM users_events WHERE user_id = $1 AND users_events.paid = false";
...
export const getCart: string =
  `select e.name, e.event_id, e.fee, e.date from events e
